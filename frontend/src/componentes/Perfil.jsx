...
function Perfil({ isOpen, onClose }) {
  ...
  // Sempre pega o usuário atualizado do localStorage ao abrir o modal
  useEffect(() => {
    if (!isOpen) return;
  ...
  // Atualiza a foto se o localStorage mudar (ex: após atualizar dados)
  useEffect(() => {
    function syncFoto() {
      const usuarioLogado = JSON.parse(localStorage.getItem('usuarioLogado'));
      ...
      <img
        src={
          usuario.foto_usuario
            ...
            : fotoUsuario // <- aqui é o ícone padrão
        }
        alt="Foto de perfil"
